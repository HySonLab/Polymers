...
def evaluate_loocv(X, y, name="", seed=42):
    ...
    for tr_idx, te_idx in loo.split(X):
        ...
        n_components = min(PCA_COMPONENTS, X_tr.shape[1], X_tr.shape[0] - 1)
...
# ---- Early Fusion Averaging baseline evaluation (raw embeddings) ----
def evaluate_loocv_early_avg(X_tp, X_gnn, y, name="", seed=42):
    """
    Early fusion averaging baseline:
    Per LOOCV split: StandardScaler+PCA applied separately to TP and GNN to same dim,
    then averaged, then GPR fit/predict in that averaged space.
    """
    set_all_seeds(seed)
    loo = LeaveOneOut()
    y_pred = np.zeros_like(y)
    ...
    for tr_idx, te_idx in loo.split(X_tp):
        ...
        k_tp = min(PCA_COMPONENTS, Xtp_tr.shape[1], Xtp_tr.shape[0] - 1)
        k_gn = min(PCA_COMPONENTS, Xgn_tr.shape[1], Xgn_tr.shape[0] - 1)
        k = min(k_tp, k_gn)
...
# ---- ADDED ONLY: TRUE Late Fusion (prediction-level) using RAW embeddings ----
def evaluate_loocv_true_late_fusion_raw(X_tp, X_gnn, y, alpha=0.5, name="", seed=42):
    """
    TRUE late fusion baseline:
    Per LOOCV split:
      - Fit TP branch: StandardScaler -> PCA -> GPR, predict held-out
      - Fit GNN branch: StandardScaler -> PCA -> GPR, predict held-out
      - Combine predictions: y_hat = alpha*y_hat_tp + (1-alpha)*y_hat_gnn
    Uses ORIGINAL (non-aligned) embeddings.
    """
    set_all_seeds(seed)
    loo = LeaveOneOut()
    y_pred = np.zeros_like(y)
...
def main():
    ...
    all_results = {
        ...
        'aligned_concat': [],
        'aligned_avg': [],
        'weighted': {},          # feature-level weighted in aligned space (your existing)
        'true_late_raw': {}      # ADDED ONLY: prediction-level late fusion on RAW embeddings
    }

    alphas_to_test = [0.3, 0.4, 0.5, 0.6, 0.7]
    for alpha in alphas_to_test:
        all_results['weighted'][alpha] = []
        all_results['true_late_raw'][alpha] = []   # ADDED ONLY
    ...
    for run_idx in range(NUM_RUNS):
        ...
        # Weighted fusion in aligned space (your existing block)
    ...
    # Find best alpha (aligned feature-level weighted; your existing selection)
    alpha_stats = {}
    best_alpha = None
    best_alpha_r2 = -1
    ...
    # Find best alpha (TRUE late fusion on RAW embeddings) — ADDED ONLY
    late_raw_stats = {}
    best_alpha_raw = None
    best_alpha_raw_r2 = -1
    ...
    # Latent-Space Aligned (Concat)
    rows.append({
        "Fusion Type": "Latent-Space Aligned",
        "Method": "Concatenation",
    ...
    # Late Fusion (your existing "weighted" — feature-level in aligned space)
    rows.append({
        "Fusion Type": "Late Fusion",
        "Method": f"Weighted Combination (Aligned features, α={best_alpha})",
    ...
    # TRUE Late Fusion (RAW, prediction-level) — ADDED ONLY
    rows.append({
        "Fusion Type": "Late Fusion (True)",
        "Method": f"Weighted Prediction (Raw, α={best_alpha_raw})",
